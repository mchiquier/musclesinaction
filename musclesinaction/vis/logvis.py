...
class MyLogger(logvisgen.Logger):
    ...
    def handle_train_step(self, epoch, phase, cur_step, total_step, steps_per_epoch,
        ...
        if cur_step % self.step_interval == 0:
            ...
            for j in range(len(data_retval['frame_paths'][0])):
                ...
                current_path = self.visualize_video(framelist,data_retval['2dskeleton'][j],cur_step,j,phase)
                threedskeleton = data_retval['3dskeleton'][j]
                current_path = self.visualize_skeleton(threedskeleton,data_retval['bboxes'][j],data_retval['predcam'][j],cur_step,j,phase)
                ...
                command = ['ffmpeg', '-i', f'{current_path}/out.mp4', '-i',f'{current_path}/out3dskeleton.mp4',  '-i', f'{current_path}/epoch_0_leftleg_emg.mp4','-filter_complex',
    ...
    def visualize_video(self, frames, twodskeleton, cur_step,j,phase):

        current_path = "../../www-data/mia/muscleresults/" + self.args.name + '_' + phase + '_viz_digitized/' + str(cur_step) + "/" + str(j)
    ...
    def visualize_skeleton(self, threedskeleton, bboxes, predcam, cur_step,ex,phase):
        ...
        current_path = "../../www-data/mia/muscleresults/" + self.args.name + '_' + phase + '_viz_digitized/' + str(cur_step) + "/" + str(ex)
    ...
    def handle_val_step(self, epoch, phase, cur_step, total_step, steps_per_epoch,
            ...
            if cur_step % self.step_interval == 0:
                ...
                current_path = self.visualize_video(framelist,data_retval['2dskeleton'][0],cur_step,phase)
                threedskeleton = data_retval['3dskeleton'][0]
                current_path = self.visualize_skeleton(threedskeleton,data_retval['bboxes'][0],data_retval['predcam'][0],cur_step,phase)
                ...
                if self.classif:
                    ...
                    gt_values = data_retval['left_quad'][0]
                    gt_values[gt_values>100.0] = 100.0
                    pred_values = model_retval['emg_output'][0][0].cpu()*100.0
                    self.animate([gt_values.numpy()],[pred_values.detach().numpy()],['left_quad'],'leftleg',2,current_path,epoch)
