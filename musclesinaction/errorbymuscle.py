...
def main(args, logger):
    ...
    for cur_step, data_retval in enumerate(tqdm.tqdm(val_aug_loader)):
            ...
            list_of_pred_emg_cur.append(emg_output[:,:,:].detach().numpy())
            list_of_val_emg_cur.append(emggroundtruth.numpy())
    
    np_val_emg_cur = np.array(list_of_val_emg_cur)*100
    np_pred_emg_cur = np.array(list_of_pred_emg_cur)*100
    for i in range(np_val_emg_cur.shape[2]):
        print(msel(torch.tensor(np_val_emg_cur[:,:,i,:])*100,torch.tensor(np_pred_emg_cur[:,:,i,:])*100), i)
    pdb.set_trace()
    ...
    print(msel(torch.tensor(np_val_emg_cur)*100,torch.tensor(np_pred_emg_cur)*100))
