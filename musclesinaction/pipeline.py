...
class MyTrainPipeline(torch.nn.Module):
    ...
    def forward(self, data_retval, cur_step, total_step):
        ...
        twodkpts = twodkpts.reshape(twodskeleton.shape[0],30,twodkpts.shape[1],twodkpts.shape[2])
        ...
        #print(cur2-cur,"2")
        cur = cur2
        if self.train_args.threed and self.train_args.transformer:
            emg_output = self.my_model(skeleton, bined_left_quad)
        if not self.train_args.threed and self.train_args.transformer:
            emg_output = self.my_model(twodskeleton, bined_left_quad)
        if not self.train_args.threed and not self.train_args.transformer:
        ...
        if not self.train_args.classif:
            loss = self.mse(emg_output[:,0,0,:], leftquad.type(torch.cuda.FloatTensor))
            #print(leftquad[:,:])
        else:
